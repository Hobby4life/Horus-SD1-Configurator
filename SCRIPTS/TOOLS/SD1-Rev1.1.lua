...
local function redrawFieldsPage()
  lcd.clear()
    ...
    lcd.drawText(1,5, "Horus SD1 Configurator", MENU_TITLE_COLOR)
  ...
  for index = 1, 7, 1 do
    ...
    lcd.drawText(1, 32*index, field[1])

    if field[4] == nil then
      lcd.drawText(COLUMN_2, 1+32*index, "---", attr)--LEFT + 
    else
      if field[2] == VALUE then
        lcd.drawNumber(COLUMN_2, 1+32*index, field[4], attr)
      elseif field[2] == COMBO then
        if field[4] >= 0 and field[4] < #(field[5]) then
          lcd.drawText(COLUMN_2, 1+32*index, field[5][1+field[4]], attr)
        end
      end
    end
  end
